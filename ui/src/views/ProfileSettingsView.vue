...
function handleAvatarUpload(event: any) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    toast.add({ severity: 'warn', summary: 'Datei zu groß', detail: 'Bitte wähle ein Bild unter 5MB', life: 3000 })
    return
  }
...
async function handleSave() {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Gespeichert', detail: 'Profil erfolgreich aktualisiert', life: 3000 })
  } catch {
    toast.add({ severity: 'error', summary: 'Fehler', detail: 'Profil konnte nicht gespeichert werden', life: 3000 })
  } finally {
    loading.value = false
  }
}
        ...
        <template #content>
          <div class="flex flex-column gap-5">
            <div class="flex flex-column align-items-center gap-3 pb-4 avatar-section">
              ...
              <p class="text-xs text-color-secondary m-0">Erlaubte Formate: JPG, PNG, GIF (max. 5MB)</p>
            </div>

            <div class="flex flex-column gap-4">
              <div class="flex flex-column gap-2">
                <label class="font-semibold">Benutzername</label>
                <InputText :value="authStore.user?.username" :disabled="true" />
              </div>

              <div class="flex flex-column gap-2">
                <label for="email" class="font-semibold">E-Mail</label>
                <InputText id="email" v-model="email" type="email" placeholder="deine@email.de" :disabled="loading" />
              </div>
              ...
              <div class="grid">
                <div class="col-12 md:col-6">
                  <div class="flex flex-column gap-2">
                    ...
                    <InputText id="firstName" v-model="firstName" placeholder="Max" :disabled="loading" />
                  </div>
                </div>
                <div class="col-12 md:col-6">
                  <div class="flex flex-column gap-2">
                    <label for="lastName" class="font-semibold">Nachname</label>
                    <InputText id="lastName" v-model="lastName" placeholder="Mustermann" :disabled="loading" />
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-column gap-2">
                ...
                <InputText id="street" v-model="street" placeholder="Musterstraße 123" :disabled="loading" />
              </div>
              ...
              <div class="grid">
                <div class="col-12 md:col-4">
                  <div class="flex flex-column gap-2">
                    ...
                    <InputText id="zipCode" v-model="zipCode" placeholder="44137" :disabled="loading" />
                  </div>
                </div>
                <div class="col-12 md:col-8">
                  <div class="flex flex-column gap-2">
                    <label for="city" class="font-semibold">Stadt</label>
                    <InputText id="city" v-model="city" placeholder="Dortmund" :disabled="loading" />
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-column gap-2">
                ...
                <InputText id="country" v-model="country" placeholder="Deutschland" :disabled="loading" />
              </div>

              <div class="flex gap-2 pt-3">
                <Button label="Speichern" icon="pi pi-check" :loading="loading" @click="handleSave" />
                <Button label="Abbrechen" icon="pi pi-times" severity="secondary" outlined @click="router.push('/settings')" />
              </div>
            </div>
          </div>
        </template>
